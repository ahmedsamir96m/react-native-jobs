...
const JobDetails = () => {
	...
	const { data, isLoading, isError, refetch } = useFetch('job-details', { job_id: params.id, })
	...
	return (
		<SafeAreaView style={{ flex: 1, backgroundColor: COLORS.lightWhite }}>
			<Stack.Screen options={{
				...
				headerLeft: () => (<ScreenHeaderBtn iconUrl={icons.left}
					dimension="60%" handlePress={() => router.back()} />)
				, headerRight: () => (<ScreenHeaderBtn iconUrl={icons.share}
					dimension="60%" handlePress={() => router.back()} />)
			}} />
			...
			<>
				...
				>
					{isLoading ? <ActivityIndicator size='large' color={COLORS.primary} /> : isError ?
